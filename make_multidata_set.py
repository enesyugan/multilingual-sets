...
parser.add_argument('-langs', nargs='+', help="language tags of each set i.e. de en", type=str, required=True)
parser.add_argument('-mix_style', help='def: utt, or span ', default="utt")
...
parser.add_argument('-textidx', help='column index of target transcript', type=int, default=5)
parser.add_argument('-outa', help="output audio file", type=str)
parser.add_argument('-outt', help="output transcript file", type=str)
parser.add_argument('-outl', help="output list of audio order file", type=str)
args = parser.parse_args()

assert len(args.stms) == len(args.langs)
random.seed(66)

stm_sets = {}
used_ids = {}
for lang, stm_file in zip(args.langs, args.stms):
    stm_sets[lang] = list()
    with open(stm_file, "r") as stmf:
        for line  in stmf:
            ...
            text = tokens[args.textidx]
            wav_path = tokens[args.wavidx]
            tmp = list()
            tmp.append((uid, wav_path, text))
            stm_sets[lang].append(tmp)
            used_ids[lang] = {}
...
generate_set(stm_sets, used_ids, args)
